...
<body style="background-color:#29e3b8;">
    ...
    <center>
        <div class="container" style="margin-top: 10%; padding:10px 80px 10px 80px; ">
            ...
            <div class="app-form-group">
              <input class="app-form-control" placeholder="ACCOUNT NO." type="number" name="AccountNo" required>
            </div>
            <br>
...
if(!$conn){
    ...
    if(isset($_POST['submit'])){
        ...
        $sql="insert into customers(name,email,balance,AccountNo) values('{$name}','{$email}','{$balance}','{$accno}')";
        $result=mysqli_query($conn,$sql);
        if($result){
                   echo "<script> alert('User created.Now you can transfer the money');
