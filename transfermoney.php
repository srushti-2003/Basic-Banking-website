...
<body style="background-color:#29e3b8; padding-top:8%;">
    <style>
        .nav-link:hover {
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed fixed-top"
        style="font-family:candara; font-size:18px; box-shadow: 0px 0px 10px 0px grey;">
        <div class="container-fluid">            <a href="index.php"><img src="statics\logo.png" alt=""
            ...
            <b>
                <ul class="navbar-nav ">
                    ...
                    <li class="nav-item" style="margin-left:15px; ">
                        <a class="nav-link text-nowrap active" href="transfermoney.php">Transfer Money</a>
                    </li>
                    <li class="nav-item" style="margin-left:15px;">
        ...
        <div class="container" style="margin-top:2%;">
            ...
            <div class="" style=" backdrop-filter: blur(5px);  border-radius:5px;  ">
                <form action="transfermoney.php" method="post">
                    ...
                    <p style="color:white;">Want to check your balance? check <a href="checkbalance.php">here</a></p>
                </form>
            </div>
        </div>
...
if($_SERVER['REQUEST_METHOD']== 'POST'){
    ...
    $checkblcquery = "SELECT Balance FROM customers where AccountNo='$sender'";
    $checkblc = mysqli_query($conn, $checkblcquery);
    $ava_blc = mysqli_fetch_assoc($checkblc)['Balance'];

    if($ava_blc >= $amount){
    $sql1 = "UPDATE customers SET Balance= Balance-$amount WHERE AccountNo='$sender'";
    $sql2 = "UPDATE customers SET Balance= Balance+$amount WHERE AccountNo='$reciever'";
    ...
    if($result1 && $result2){
        ...
        <h2><i class="fas fa-check-circle"></i>
          Transaction Successful!</h2></div>
        </div>
      </div>';

      $sqltran = "INSERT INTO `transactions` (`Sender`, `Receiver`, `Amount`) VALUES ('$sender', '$reciever', '$amount')";
